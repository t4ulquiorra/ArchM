...
fun YouTubeSelectionSongMenu(
    ...
    val listenTogetherManager = com.archm.player.LocalListenTogetherManager.current
    ...
    AddToPlaylistDialogOnline(
        ...
        songs = remember { 
            songSelection.map { song ->
                ...
                val metadata = song.toMediaMetadata()
                com.archm.player.db.entities.Song(
                    song = com.archm.player.db.entities.SongEntity(
                        id = metadata.id,
                    ...
                    artists = metadata.artists.map { artist ->
                        com.archm.player.db.entities.ArtistEntity(
                            id = artist.id ?: "",
                            name = artist.name
                        )
                    },
                    album = metadata.album?.let { album ->
                        com.archm.player.db.entities.AlbumEntity(
                            id = album.id,
                            title = album.title,
                            thumbnailUrl = metadata.thumbnailUrl, 
                            songCount = 0,
                            ...
                            database.transaction {
                                songSelection.forEach { song ->
                                    ...
                                    if ((!allLiked && !metadata.liked) || allLiked) {
                                        ...
                                        val songEntity = com.archm.player.db.entities.SongEntity(
                                            id = metadata.id,
                                            title = metadata.title,
                                            duration = metadata.duration,
                                            thumbnailUrl = metadata.thumbnailUrl,
