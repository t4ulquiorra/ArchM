...
import com.archm.player.constants.AutomixDebugOverlayKey
...
import com.archm.player.constants.EnableGoogleCastKey
import com.archm.player.constants.HistoryDuration
import com.archm.player.constants.KeepScreenOn
...
import com.archm.player.constants.SeekExtraSeconds
...
highlightKey: String? = null) {
    ...
    val (dataSaverEnabled, onDataSaverEnabledChange) = rememberPreference(
        key = com.archm.player.constants.DataSaverEnabledKey,
        defaultValue = false
    )
    ...
    val (playbackEngine, onPlaybackEngineChange) = rememberEnumPreference(
        com.archm.player.constants.PlaybackEngineKey,
        defaultValue = com.archm.player.constants.PlaybackEngine.AUTO
    )

    val (downloadQuality, onDownloadQualityChange) = rememberEnumPreference(
        com.archm.player.constants.DownloadQualityKey,
        defaultValue = com.archm.player.constants.DownloadQuality.YOUTUBE
    )
    ...
    if (showDownloadQualityDialog) {
        EnumDialog(
            ...
            values = listOf(com.archm.player.constants.DownloadQuality.YOUTUBE),
            valueText = {
                when (it) {
                    com.archm.player.constants.DownloadQuality.YOUTUBE -> "YouTube Music (AAC/Default)"
                    else -> ""
                }
            }
        )
    ...
    if (showPlaybackEngineDialog) {
        EnumDialog(
            ...
            onSelect = {
                ...
                com.archm.player.utils.YTPlayerUtils.playbackEngine = it
                showPlaybackEngineDialog = false
            },
            title = "Playback Engine",
            current = playbackEngine,
            values = listOf(
                com.archm.player.constants.PlaybackEngine.POTOKEN,
                com.archm.player.constants.PlaybackEngine.BRAVEPIPE,
                com.archm.player.constants.PlaybackEngine.AUTO,
            ),
            valueText = {
                when (it) {
                    com.archm.player.constants.PlaybackEngine.POTOKEN -> "PoToken (Recommended)"
                    com.archm.player.constants.PlaybackEngine.BRAVEPIPE -> "BravePipe (NewPipe)"
                    com.archm.player.constants.PlaybackEngine.AUTO -> "Auto (Try Both)"
                }
            },
            valueDescription = {
                when (it) {
                    com.archm.player.constants.PlaybackEngine.POTOKEN -> "Uses WebView PoToken + CipherDeobfuscator. Most reliable and future-proof."
                    com.archm.player.constants.PlaybackEngine.BRAVEPIPE -> "Uses NewPipe extractor for stream resolution. Lightweight but may break with YouTube updates."
                    com.archm.player.constants.PlaybackEngine.AUTO -> "Tries PoToken first, falls back to BravePipe if it fails."
                }
            }
        )
    }
